...
RSpec.describe Transaction, type: :model do
  ...
  let(:transaction_top_up) { FactoryBot.build(:transaction, income_amount: 1000, type: 'top_up', asset_type: 'cash', user_id: user.id) }
  ...
  describe "Validations" do
    ...
    describe "#cash_enough" do
      it "should create withdraw transaction successfully" do
        transaction_withdraw = FactoryBot.build(:transaction, income_amount: 500, type: 'withdraw', user_id: user.id)
      ...
      it "should create withdraw transaction failured" do
        transaction_withdraw = FactoryBot.build(:transaction, income_amount: 1500, type: 'withdraw', user_id: user.id)
        transaction_withdraw.save
      ...
      it "should create buy transaction successfully with asset" do
        transaction_buy = FactoryBot.build(:transaction, income_amount: 50, type: 'buy', asset_type: 'gold', user_id: user.id)
      ...
      it "should create buy transaction failured with asset" do
        transaction_buy = FactoryBot.build(:transaction, income_amount: 200, type: 'buy', asset_type: 'gold', user_id: user.id)
        transaction_buy.save
    ...
    describe "#asset_enough" do
      it "should create sell transaction successfully" do
        transaction_buy = FactoryBot.build(:transaction, income_amount: 50, type: 'buy', asset_type: 'gold', user_id: user.id)
        transaction_buy.save
        transaction_sell = FactoryBot.build(:transaction, income_amount: 20, asset_type: 'gold', type: 'sell', user_id: user.id)
  ...
  describe "Methods" do
    describe "#asset_balance" do
      it "should return balance from new asset correctly" do
        ...
        transaction_buy = FactoryBot.build(:transaction, income_amount: 10, type: 'buy', asset_type: 'silver', user_id: user.id)
    ...
    describe "#relate_to_balance" do
      ...
      context "with asset type" do
        ...
        it "should relate to correct balance with sell gold" do
          transaction_buy = FactoryBot.build(:transaction, income_amount: 50, type: 'buy', asset_type: 'gold', user_id: user.id)
          transaction_buy.save
          transaction_sell = FactoryBot.build(:transaction, income_amount: 50, type: 'sell', asset_type: 'gold', user_id: user.id)
          transaction_sell.save
          cash_balance = user.balances.where(asset_id: asset_cash.id).first
          gold_balance = user.balances.where(asset_id: asset_gold.id).first
